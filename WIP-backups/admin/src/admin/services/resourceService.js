...
import { uploadResourceCoverImage } from './resourceImageService';
...
async function resolveCoverImage(payload) {
  const file = payload?.coverImageFile || payload?.cover_image_file || null;
  if (file) {
    const uploadResult = await uploadResourceCoverImage(file);
    return uploadResult?.publicUrl || null;
  }

  return payload?.coverImage || payload?.cover_image || null;
}
...
export const resourceService = {
  ...
  async createResource(payload) {
    try {
      ...
      try {
        resolvedCoverImage = await resolveCoverImage(payload);
      } catch (error) {
        return buildErrorResponse({ message: error?.message || 'Failed to upload the cover image.' });
      }
      ...
      const mapped = buildResourcePayload(payloadWithImage);
      if (mapped.error) {
