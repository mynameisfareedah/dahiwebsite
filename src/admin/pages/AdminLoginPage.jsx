...
export default function AdminLoginPage() {
  ...
  const getFriendlyError = (message) => {
    ...
    if (normalized.includes('invalid login credentials') || normalized.includes('invalid credentials') || normalized.includes('invalid login')) {
    ...
    if (normalized.includes('not configured')) {
      return 'Supabase is not configured. Please contact your administrator.';
    }
    ...
    if (normalized.includes('not authorized') || normalized.includes('not authorised')) {
      return 'You are authenticated but are not authorized to access the admin dashboard.';
    }
  ...
  const authError = error || authContextError;
  const displayedError = getFriendlyError(authError);
        ...
        <div className="flex-1 bg-slate-50 p-8 text-slate-700 sm:p-10">
          ...
          {displayedError && !success && (
            <div className="mb-6 flex items-start gap-3 rounded-lg border border-red-200 bg-red-50 p-4">
              ...
              <div>
                <p className="text-sm font-medium text-red-900">{displayedError}</p>
                {(displayedError.includes('invalid credentials') || displayedError.includes('authenticated but are not authorized')) && (
                  <p className="mt-1 text-xs text-red-700">
                    Please contact your administrator if you need access to the admin dashboard.
                  </p>
                )}
              </div>
            </div>
