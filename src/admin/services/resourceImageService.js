...
export async function uploadResourceCoverImage(file) {
  ...
  if (error) {
    try {
      ...
      console.error('STORAGE ERROR (no user):', { error, bucket: STORAGE_BUCKET, filePath, data });
    }

    const storageErr = buildStorageError(error);
    const message = storageErr?.message || (error?.message || JSON.stringify(error));
